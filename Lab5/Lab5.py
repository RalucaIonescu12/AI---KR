...
class Graph:  # graful problemei
   ...
   def succesori(self, nodCurent):
       ...
       for (dx, dy) in directii:
            ...
            if lPlacuta >= 0 and lPlacuta < 3 and cPlacuta >= 0 and cPlacuta < 3:
                ...
                nodNou = NodParcurgere(stareNoua, nodCurent.g + 1, 0, nodCurent)
   ...
   def valideaza(self):
        if len(self.start)!=3:
            return False
        if not all([len(linie)==3 for linie in self.start]):
            return False
        matrDesf =sum(self.start, start=[])
        if sorted(matrDesf)!=list(range(9)):
            return False
        nrInv=0
        for i in range(len(matrDesf)):
            if matrDesf[i]==0:
                continue
            for j in range(i+1,len(matrDesf)):
                if matrDesf[i] == 0:
                    continue
                if matrDesf[i]>matrDesf[j]:
                    nrInv+=1
        if nrInv % 2 == 1:
            return False
        return True
...
f=open("input.txt","r")
start = [[int(i) for i in linie.strip().split()] for linie in f.read().strip().split("\n")]
# print(start)
